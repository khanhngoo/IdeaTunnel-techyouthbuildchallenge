...
import { NODE_HEIGHT_PX, NODE_WIDTH_PX } from '../../components/constants'
...
export abstract class NodeDefinition<Node extends { type: string }> {
	...
	onPortConnect(shape: NodeShape, node: Node, port: PortId): void {
		void shape;
		void node;
		void port;
	}
	onPortDisconnect(shape: NodeShape, node: Node, port: PortId): void {
		void shape;
		void node;
		void port;
	}
